...
async fn main() {
    ...
    // if $HOSTER_CHAT_ID is provided a message is sent to the hoster of the bot
    if let Ok(hoster_chat_id) = env::var("HOSTER_CHAT_ID") {
        bot.send_message(hoster_chat_id, "Bot running".to_string())
            .await
